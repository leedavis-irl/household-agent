...
export const definition = {
  name: 'email_send',
  description:
    "Send an email from a household member's Gmail account. Defaults to the person speaking.",
  input_schema: {
    type: 'object',
    properties: {
      person: {
        type: 'string',
        description: "Whose Gmail to send from (person id or display name). Default: the person speaking.",
      },
      to: {
        type: 'string',
...
export async function execute(input, envelope) {
  ...
  if (!personId) {
    return { error: 'Could not identify whose email to send from. Use person id (e.g. lee, steve).' };
  }

  if (!checkEmailSendPermission(personId, envelope)) {
    return {
      error: `Permission denied: ${envelope.person} cannot send email as ${personId}.`,
    };
  }
  ...
  try {
    ...
    log.error('Email send: getClient failed', { personId, error: err.message });
  ...
  try {
    ...
    log.error('Email send failed', { personId, error: err.message });
