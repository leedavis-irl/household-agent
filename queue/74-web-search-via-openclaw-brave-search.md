...
**Sphere:** Engine
**Backlog item:** Web search via OpenClaw/Brave Search
**Depends on:** TBD — review existing tools and integrations before starting

## What to build

Gives Iji the ability to search the web by routing requests through OpenClaw's existing Brave Search integration rather than building a separate EC2 integration.
...
```
./scripts/gh-update-card.sh "Web search via OpenClaw/Brave Search" "In Review"
```

## Commit message

`feat: web search via openclaw brave search`
